...
const services = [
  { number: '01', title: 'Product discovery & UX', text: 'Turn an unclear requirement into prioritized workflows, a delivery roadmap, interaction design, and a testable first release.', tags: ['Workshops', 'User flows', 'Prototypes', 'MVP scope'], href: '/contact' },
  { number: '02', title: 'Custom software development', text: 'Build portals, dashboards, marketplaces, operational tools, e-commerce experiences, and customer-facing platforms around real business rules.', tags: ['React', 'Node.js', '.NET', 'Python'], href: '/services/custom-software-development' },
  { number: '03', title: 'SaaS product engineering', text: 'Engineer multi-tenant products with teams, roles, subscriptions, usage, notifications, administration, analytics, and resilient cloud foundations.', tags: ['Multi-tenancy', 'Billing', 'RBAC', 'Analytics'], href: '/services/saas-development' },
  { number: '04', title: 'React Native mobile apps', text: 'Deliver cross-platform mobile products with secure APIs, offline behavior, notifications, payments, and store-ready release workflows.', tags: ['Android', 'Offline data', 'Push'], href: '/services/mobile-app-development' },
  { number: '05', title: 'AI automation & agents', text: 'Connect models to approved data and tools with retrieval, evaluations, human controls, observability, and clear operational ownership.', tags: ['RAG', 'Agents', 'Evaluation', 'Guardrails'], href: '/services/ai-automation' },
  { number: '06', title: 'Cloud, DevOps & modernization', text: 'Improve deployment, reliability, monitoring, recovery, security, performance, and legacy systems without disrupting critical operations.', tags: ['AWS', 'Docker', 'CI/CD', 'Observability'], href: '/services/cloud-devops-modernization' },
];
...
function ServicesPage() {
  return <>
    ...
    <section className="service-ledger">{services.map((service) => <article key={service.number}><span className="service-ledger__number">{service.number}</span><div><h2>{service.title}</h2><p>{service.text}</p></div><ul>{service.tags.map((tag) => <li key={tag}>{tag}</li>)}</ul><a href={service.href} aria-label={`Explore ${service.title}`}><ArrowUpRight size={20} /></a></article>)}</section>
